from typing import Dict, List, Literal, Optional, TypeVar, Generic, Union
...
class ServingSizes(BaseModel):
    ...
    breakfast: Optional[MaxMin[MealSizeEnum]] = None
    lunch: Optional[MaxMin[MealSizeEnum]] = None
    dinner: Optional[MaxMin[MealSizeEnum]] = None
    snacks: Optional[MaxMin[MealSizeEnum]] = None
    lunch_desert: Optional[MaxMin[MealSizeEnum]] = None
    dinner_desert: Optional[MaxMin[MealSizeEnum]] = None
...
# TODO: create a meal plan constraints class that inherits from the day constraints class that inherits from the recipe constraints class
class RecipeConstraints(BaseModel):
    calories: Calories = Field(
        description="A single meal's caloric value limits")
    protein: Protein = Field(
        description="A single meal's protein limits in grams")
    lipids_pcnt_non_protein_calories: LipidsPcntNonProteinCalories = Field(
        description="Percentage of a single meal's non-protein calories from lipids")
    ...
    cooking_time_minutes: CookingTimeMinutes = Field(
        description="Acceptable cooking time range in minutes")
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> 'RecipeConstraints':
        return cls(**data)

    def copy_with(self, **kwargs) -> 'RecipeConstraints':
        return self.model_copy(update=kwargs)

    @classmethod
    def default(cls) -> 'RecipeConstraints':
        """
...
class DayConstraints(BaseModel):
    ...
    @classmethod
    def from_dict(cls, data: Dict) -> 'DayConstraints':
        return cls(**data)

    def copy_with(self, **kwargs) -> 'DayConstraints':
        return self.model_copy(update=kwargs)

    @classmethod
    def default(cls) -> 'DayConstraints':
        """
        Default constraints for a singl meal for a healthy adult male
        """
        return cls(
...
ConstraintsType = Union[RecipeConstraints, DayConstraints, MealPlanConstraints]
